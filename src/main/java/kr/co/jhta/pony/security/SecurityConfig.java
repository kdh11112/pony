...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.cors().and().csrf().disable().authorizeRequests()
				...
				.antMatchers("/login", "/", "/ponyreg", "/ponyRegOk", "/model/**", "/admin", "/adminlogin").permitAll() // 로그인
																														// 페이지와
																														// 기본
																														// 페이지는
																														// 인증
...
//                .userDetailsService(customUserDetailsService)
	...
	private String determineTargetUrl(HttpServletRequest request, Authentication authentication) {
		...
		System.out.println("지나가니2" + requestUri);
		// 요청한 페이지가 /adminlogin이면 /admin으로, 그 외에는 /로 리다이렉트
		if (requestUri.equals("/adminlogin")) {
			targetUrl = "/admin";
		}
